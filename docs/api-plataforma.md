...
**Solo para lo que vive fuera de este servidor.** Un flujo que ya corre dentro
de esta aplicación —un aviso al cambiar un ticket, el agente de IA, un comando
de consola— usa `ConversationSender` directamente. Hacer que Laravel se llame a
sí mismo por HTTP con un Bearer token añade un secreto que rotar, una llamada
de red que puede fallar y un punto más donde autenticarse, a cambio de nada.

Cada credencial emitida es una credencial que hay que custodiar, rotar y dar de
baja. Ver `docs/avisos-whatsapp-tickets.md` como ejemplo del camino interno.
