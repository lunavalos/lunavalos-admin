...
const submit = () => {
    form.post(route('signature-templates.store'));
};
...
<template>
    <Head title="Crear Plantilla de Firma" />
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="grid grid-cols-12 gap-8">
                                <div class="col-span-12 lg:col-span-7 space-y-4">
                                    <div class="flex items-center justify-between">
                                        <InputLabel for="html_content" value="Editor de Código" class="!mb-0" />
                                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">HTML / TEXT</span>
                                    </div>
                                    <p class="text-xs text-gray-500">Usa variables como: {{name}}, {{position}}, {{email}}, {{phone}}, ...</p>
                                ...
                                <div class="col-span-12 lg:col-span-5 space-y-4">
                                    <div class="flex items-center justify-between">
                                        <InputLabel value="Vista Previa en Vivo" class="!mb-0" />
                                        <span class="text-[10px] font-bold text-blue-500 uppercase tracking-widest animate-pulse">Live</span>
                                    </div>
                                    <div class="p-8 bg-white border border-gray-200 rounded-xl min-h-[400px] shadow-inner overflow-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]">
                            ...
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" v-model="form.is_active" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 h-5 w-5" />
                                    <span class="ml-2 text-sm text-gray-700 font-bold">Plantilla Activa</span>
                                </label>
                            </div>

                            <div class="flex items-center justify-end">
                                <Link :href="route('signature-templates.index')" class="text-gray-600 mr-4 hover:underline text-sm">Cancelar</Link>
